...
class AxisLength extends AnimationBuilderItem {
  #obj;
  #interpolator_positive_x;
  #interpolator_positive_y;
  #interpolator_negative_x;
  #interpolator_negative_y;
  #length;
  #start;
  #from;
  #to;
  ...
  onDrawFrame(relativeFrameCount, _parent) {
    this.#obj.axisPositiveXLength = this.#interpolator_positive_x.interpolate((relativeFrameCount + 1) / this.#length);
    this.#obj.axisPositiveYLength = this.#interpolator_positive_y.interpolate((relativeFrameCount + 1) / this.#length);
    this.#obj.axisNegativeXLength = this.#interpolator_negative_x.interpolate((relativeFrameCount + 1) / this.#length);
    this.#obj.axisNegativeYLength = this.#interpolator_negative_y.interpolate((relativeFrameCount + 1) / this.#length);
  }
  get startFrame() {
    return this.#start;
  }
...
class HTMLPlugin extends Carobj {
  #svg_content;
  #content;
  #img = document.createElement("img");
  #DOMURL = window.URL || window.webkitURL || window;
  constructor(content, datas) {
    super(datas);
    this.#content = content;
    this.#svg_content = '<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200">' + '<foreignObject width="100%" height="100%">' + '<div xmlns="http://www.w3.org/1999/xhtml">' + content + '</div>' + '</foreignObject>' + '</svg>';
  }
  onDraw(ctx, element) {
  ...
  set content(value) {
    this.#content = value;
    this.#svg_content = '<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200">' + '<foreignObject width="100%" height="100%">' + '<div xmlns="http://www.w3.org/1999/xhtml">' + value + '</div>' + '</foreignObject>' + '</svg>';
  }
}
...
export { AudioItem, Car, animation, interpolator, object };
